...
elseif(isempty(position))
  ...
  if (min(thisPlot) < 1)&&0 % negative Thisplot corresponds to
    ...
    if ~strcmp(get(gcf,'defaultaxesunits'),'normalized')
      ...
      def_pos = get(gcf,'DefaultAxesPosition')+[-.1 -.18 +.1 +.05];
    end
    ...
    height = totalheight/nrows*(max(row)-min(row)+1)-row_offset + 0.3;
...
if(kill_siblings)
  ...
  for i1 = 1:length(sibs)  
    ...
    if(strcmp(get(sibs(i1),'Type'),'axes'))
      ...
      if intersect
	if sibpos(2) < position(2) || got_one || any(abs(sibpos - position) > tol)  
	  %delete(sibs(i1));
      position(2) = sibpos(2)-(sibpos(4)*1.5);
	else
	  got_one = 1;
